...
// Returns y after last row
function drawHorizBars(
  pdf: jsPDF,
  data: Array<{ name: string; count: number; pct: number; color?: RGB }>,
  x: number, y: number, w: number,
  defaultColor: RGB = C.blue,
  maxRows = 14,
): number {
  const ROW_H   = 7
  const LABEL_W = Math.min(55, Math.floor(w * 0.52))
  const PCT_W   = 14
  const BAR_W   = Math.max(w - LABEL_W - PCT_W - 4, 8)

  data.slice(0, maxRows).forEach((d, i) => {
    const bg: RGB = i % 2 === 0 ? C.white : C.gray100
    fc(pdf, bg); pdf.rect(x, y, w, ROW_H, 'F')

    tc(pdf, C.gray700); pdf.setFont('helvetica', 'normal'); pdf.setFontSize(5.5)
    const lbl = d.name.length > 28 ? d.name.slice(0, 25) + '…' : d.name
    pdf.text(lbl, x + 2, y + 5)

    const bx  = x + LABEL_W
    const col = d.color ?? defaultColor
    fc(pdf, C.gray200); pdf.roundedRect(bx, y + 1.5, BAR_W, ROW_H - 3, 1, 1, 'F')
    const fill = BAR_W * (Math.min(d.pct, 100) / 100)
    if (fill > 0.5) { fc(pdf, col); pdf.roundedRect(bx, y + 1.5, fill, ROW_H - 3, 1, 1, 'F') }

    tc(pdf, col); pdf.setFont('helvetica', 'bold'); pdf.setFontSize(5.5)
    pdf.text(`${d.pct}%`, bx + BAR_W + 2, y + 5)

    y += ROW_H
  })
  dc(pdf, C.gray200); pdf.line(x, y, x + w, y)
  return y
}
...
function generateDashboardPdfLegacy(p: DashboardPdfParams) {
  ...
  const statusRows = [
    ...p.statusPie.map(s => [
      ...
      { text: statusTotal > 0 ? `${Math.round(s.value / statusTotal * 100)}%` : '0%', color: C.gray500 },
    ]),
    [
    ...
    ...['on_time','overdue_1_7','overdue_8_30','overdue_30_plus','no_target']
      .map(k => {
        ...
        return [
          ...
          { text: agingTotal > 0 ? `${Math.round(v / agingTotal * 100)}%` : '0%', color: C.gray500 },
        ]
      }),
    [
      { text: 'Total', bold: true, color: C.slate },
    ...
    ...(['High','Medium','Low'] as const)
      .map(level => {
        ...
        return [
          ...
          { text: String(v), color: C.gray700 },
          { text: riskTotal > 0 ? `${Math.round(v / riskTotal * 100)}%` : '0%', color: C.gray500 },
        ]
      }),
    [
      { text: 'Total', bold: true, color: C.slate },
  ...
  if (projRows.length === 0) {
    ...
    const pRows = projRows.slice(0, maxProjRows).map((r: any, i: number) => {
      ...
      return [
        ...
        { text: sc != null ? `${sc}%` : 'N/A', bold: true, color: gc, pill: false },
      ]
    }) as any[]
  ...
  if (obsRows.length === 0) {
    ...
    const oRows = obsRows.slice(0, maxObsRows).map((r: any, i: number) => {
      const rankColor: RGB = i === 0 ? C.amber : i < 3 ? C.blue : C.gray400
      const countColor: RGB = i === 0 ? C.amber : i < 3 ? C.blue : C.green
      return [
        { text: String(i + 1), color: rankColor, bold: i < 3 },
        { text: (r.observer_name || 'Unknown').slice(0, 30), bold: i < 3, color: i < 3 ? C.gray900 : C.gray700 },
        { text: String(r.count), bold: true, color: countColor },
      ]
    }) as any[]
